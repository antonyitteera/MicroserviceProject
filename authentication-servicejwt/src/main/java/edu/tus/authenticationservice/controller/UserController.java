   ...
   @PostMapping("/login")
    public ResponseEntity<?> loginUser(@RequestBody LoginForm loginForm)
    {
    	try
    	{
	    	...
	    	UserEntity user=userRepository.findById(Long.valueOf(loginForm.getUserid())).get();
    	...
    	catch(Exception e)
    	{
    		...
    		respObj.put("Message", "Incorrect user name or password");
    		return ResponseEntity.status(500).body(respObj);
    	}
    }
    public void authenticate(LoginForm loginForm) throws Exception
    {
    	try
    	{
    		authManager.authenticate(new UsernamePasswordAuthenticationToken(loginForm.getUserid(),loginForm.getPassword()));
    	}
